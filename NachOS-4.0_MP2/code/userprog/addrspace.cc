...
AddrSpace::AddrSpace() {
    ...
    for (int i = 0; i < NumPhysPages; i++) {
        ...
        pageTable[i].physicalPage = -1; // Have Not load the program.
...
AddrSpace::~AddrSpace() {
    // Traverse each entry and clear the loaded physical pages.
    for(int i = 0 ; i < NumPhysPages ; i++){
        if(pageTable[i].physicalPage != -1){
            kernel->isUsedPhysPages[pageTable[i].physicalPage] = 0;
...
bool AddrSpace::Load(char *fileName) {
    ...
    for(int i=0, j=0; i<numPages; i++){
        while(j<NumPhysPages && kernel->isUsedPhysPages[j]==1) j++;
        pageTable[i].physicalPage = j;
        if(pageTable[i].valid == FALSE) pageTable[i].valid = TRUE;
        kernel->isUsedPhysPages[j] = 1;
        kernel->NumAvailablePhysPages--;
    }
    ...
    if (noffH.code.size > 0) {
        ...
        if((exception = Translate(noffH.code.virtualAddr, &physicalAddr, 1)) != NoException){
    ...
    if (noffH.initData.size > 0) {
        ...
        unsigned int physicalAddr;
        if((exception = Translate(noffH.initData.virtualAddr, &physicalAddr, 1)) != NoException){
            kernel->machine->RaiseException(exception, 0);
            return FALSE;
        }
        executable->ReadAt(
    ...
    if (noffH.readonlyData.size > 0) {
        ...
        unsigned int physicalAddr;
        if((exception = Translate(noffH.readonlyData.virtualAddr, &physicalAddr, 1)) != NoException){
            kernel->machine->RaiseException(exception, 0);
            return FALSE;
        }
        executable->ReadAt(
