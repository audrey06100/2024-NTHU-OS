    ...
    switch (which) {
        case SyscallException:
            switch (type) {
                ...
                //---------------------MP1----------------------//
                case SC_Open:
                    // Load arguments from user program
                    val = kernel->machine->ReadRegister(4);
                ...
                case SC_Write:
                    ...
                    numWritten = SysWrite(buffer, numChar, fileID);

                    // Prepare Result 
                    kernel->machine->WriteRegister(2, (int)numWritten);
                ...
                case SC_Read:
                    ...
                    // Process SysWrite Systemcall
                    buffer = &(kernel->machine->mainMemory[val]);
                    numRead = SysRead(buffer, numChar, fileID);

                    // Prepare Result 
                    kernel->machine->WriteRegister(2, (int)numRead);
